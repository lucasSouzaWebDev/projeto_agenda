    	...
    	public static function searchContacts($value){
    		...
    		if($resultado != null){
    			...
    			foreach ($resultado as $key => $value) {
	    			...
	    			echo "<td class='text-center'>".preg_replace("/(\d{3})(\d{3})(\d{3})(\d{2})/", "$1.$2.$3-$4", $value['cpf'])."</td>";
	    			...
	    			if(isset($value['logradouro'])){
	    				echo "<td class='text-center'>".preg_replace("/(\d{2})(\d{3})(\d{3})/", "$1.$2-$3", $value['cep'])." - ".$value['logradouro'].", ".$value['numero']." - ".$value['bairro'].", ".$value['localidade']." - ".$value['uf']."</td>";
	    			}else{
	    				echo "<td class='text-center'>N/A</td>";	
	    			}
	    			
	    			echo "<td class='text-center'>".preg_replace("/(\d{2})(\d{4})(\d{4})/", "($1)$2-$3", $value['commercial_phone'])." - ".preg_replace("/(\d{2})(\d{4})(\d{4})/", "($1)$2-$3", $value['residential_phone'])." - ".preg_replace("/(\d{2})(\d{1})(\d{4})(\d{4})/", "($1)$2 $3-$4", $value['cell_phone'])."</td>";
